...
// Equação 1: v = 1/3 * Ab * h, onde h é a altura da pirâmide e Ab é a área do hexágono que forma a base da pirâmide. A área do hexágono
// é computada pela Equação 2: Ab = (3 * a² * raiz de 3)/2, onde a é o comprimento de uma aresta do hexágono regular.
// Entrada
...
func main() {
	var altura, aresta, volumePiramide, areaHexano = 0.0, 0.0, 0.0, 0.0

	fmt.Print("Informe a altura e em seguido o comprimento de uma aresta do hexagono: ")
	fmt.Scan(&altura, &aresta)

	areaHexano = (3 * (math.Pow(aresta, 2)) * (math.Sqrt(3))) / 2

	volumePiramide = (1.0 / 3.0) * (areaHexano * altura)

	fmt.Print(areaHexano)
