...
func main() {
	...
	// ========================================================
	//   NOVAS ROTAS: CRUD DE SISTEMA DE SAÚDE (PACIENTES)
	// ========================================================

	// 1. Rota para listar todos os pacientes (GET)
	http.HandleFunc("/pacientes", handlers.ListarPacientesHandler)

	// 2. Rota para criar um novo paciente (POST)
	http.HandleFunc("/pacientes/criar", handlers.CadastrarPacienteHandler)

	// 3. Rota para atualizar os dados de um paciente existente (PUT)
	http.HandleFunc("/pacientes/update", handlers.AtualizarPacienteHandler)

	// 4. Rota para deletar um paciente (DELETE)
	http.HandleFunc("/pacientes/delete", handlers.DeletarPacienteHandler)
